...
describe.skipIf(SKIP)("integration: AI SDK provider smoke tests", () => {
  it(
    "claude-code runner returns text from a trivial prompt",
    async () => {
  ...
  it(
    "claude-code runner maps full model IDs to short names",
    async () => {
      const runner = new ClaudeCodeRunner({
        cwd: process.cwd(),
  ...
  it(
    "gemini runner returns text from a trivial prompt",
    async () => {
